...
// add to src/api/youtubeApi.ts (append or place near other auth-based helpers)
...
export async function checkSubscriptionStatus(accessToken: string, channelId: string) {
  ...
  return res.data; // if items.length > 0 -> subscribed, items[0].id = subscriptionId
}
...
export async function subscribeToChannel(accessToken: string, channelId: string) {
  ...
  return res.data; // subscription resource (contains id)
}
